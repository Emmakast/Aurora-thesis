#!/usr/bin/env python3
"""
Visualize a grid of steered Aurora predictions across multiple alphas.
...
def plot_alpha_grid(base_ds, steered_ds_dict, var, alphas, phenomenon, level=None, output_path=None, base_file=None):
    ...
    fig.suptitle(f"{phenomenon} Steering Grid: {var.upper()}{title_suffix}", fontsize=16)
    
    # Use subplots_adjust instead of tight_layout to avoid Cartopy incompatibility warnings
...
def main():
    ...
    # Reconstruct the exact naming convention from steer_aurora.py
    ...
    for a in args.alphas:
        if args.mask_tag:
            f = data_dir / f"steered_{phenom_str}_{args.date}_{init_tag}_{args.mask_tag}_alpha_{a}.nc"
        else:
            f = data_dir / f"steered_{phenom_str}_{args.date}_{init_tag}_alpha_{a}.nc"
    ...
    plot_alpha_grid(base_ds, steered_ds_dict, 'z', args.alphas, args.phenomenon, level=50,
                    output_path=data_dir / f"grid_{phenom_str}_{args.date}_{init_tag}_z50.png", base_file=base_file)
    plot_alpha_grid(base_ds, steered_ds_dict, 'z', args.alphas, args.phenomenon, level=500,
                    output_path=data_dir / f"grid_{phenom_str}_{args.date}_{init_tag}_z500.png", base_file=base_file)
    plot_alpha_grid(base_ds, steered_ds_dict, 'msl', args.alphas, args.phenomenon,
                    output_path=data_dir / f"grid_{phenom_str}_{args.date}_{init_tag}_msl.png", base_file=base_file)
