...
ENV_FILE = "/home/ekasteleyn/aurora_thesis/thesis/steering/scripts/.env"

TARGET_DATES_CSV = "/home/ekasteleyn/aurora_thesis/thesis/steering/data/target_dates_ao_81.csv"
...
def load_attention_tensor(s3_client, year: int, month: int, day: int) -> torch.Tensor:
    ...
    # Use TMPDIR if set (e.g. from SLURM's scratch-node), otherwise fallback to high-capacity shared scratch
    tmp_base = os.environ.get('TMPDIR', '/scratch-shared/ekasteleyn/tmp')
    os.makedirs(tmp_base, exist_ok=True)
    
    with tempfile.NamedTemporaryFile(dir=tmp_base, suffix=".pt", delete=True) as tmp:
...
def plot_attention_line_plots(
    ...
    # Filter for the 13 pressure levels (skip the 4 surface variables)
    if num_physical >= 17:
        start_idx = 4
        mean_neutral_plot = mean_neutral[:, start_idx:]
        contrastive_plot = (mean_active - mean_neutral)[:, start_idx:]
        labels_plot = physical_labels[start_idx:]
        num_physical_plot = contrastive_plot.shape[1]
    else:
        mean_neutral_plot = mean_neutral
        contrastive_plot = mean_active - mean_neutral
        labels_plot = physical_labels
        num_physical_plot = num_physical
    
    # Define exact latent indices to plot (0, 1, 2)
    latent_indices = [0, 1, 2]
    
    # Colors and markers for each latent
    colors = ['#d62728', '#1f77b4', '#ff7f0e']  # Red, Blue, Orange
    markers = ['D', 'o', 's']  # Diamond, Circle, Square
    latent_names = [f'Latent {i}' for i in latent_indices]
    
    # X positions
    x = np.arange(num_physical_plot)
    ...
    for i, latent_idx in enumerate(latent_indices):
        if latent_idx < num_latents:
            ax1.plot(x, mean_neutral_plot[latent_idx], 
    ...
    ax1.set_xlim(-0.5, num_physical_plot - 0.5)
    
    # ===== Plot 2: Contrastive Difference =====
    for i, latent_idx in enumerate(latent_indices):
        if latent_idx < num_latents:
            ax2.plot(x, contrastive_plot[latent_idx], 
                     color=colors[i], marker=markers[i], markersize=6,
                     linewidth=2, label=latent_names[i])
    ...
    ax2.set_xlabel("Pressure Levels (hPa)", fontsize=12)
    ...
    ax2.set_xticks(x)
    ax2.set_xticklabels(labels_plot, rotation=45, ha='right', fontsize=10)
...
def main():
    ...
    print(f"  Storage location for downloads: {os.environ.get('TMPDIR', '/scratch-shared/ekasteleyn/tmp')}")
