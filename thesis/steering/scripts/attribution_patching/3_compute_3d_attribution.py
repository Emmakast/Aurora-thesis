...
def main():
    ...
    try:
        delta_v = torch.load(output_dir / "delta_v.pt", map_location="cpu")
        gradients = torch.load(output_dir / f"neutral_gradients_{target_date}.pt", map_location="cpu")
    ...
    vmax = np.percentile(np.abs(attribution_map), 99.9)
    vmin = -vmax
