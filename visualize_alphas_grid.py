...
def main():
    ...
    for a in args.alphas:
        f = AURORA_DIR / f"steered_{args.phenomenon.lower()}_{args.date}_alpha_{a}.nc"
        if f.exists():
    ...
    plot_alpha_grid(base_ds, steered_ds_dict, 'z', args.alphas, args.phenomenon, level=50,
                    output_path=AURORA_DIR / f"grid_{args.phenomenon.lower()}_{args.date}_z50.png")
    plot_alpha_grid(base_ds, steered_ds_dict, 'z', args.alphas, args.phenomenon, level=500,
                    output_path=AURORA_DIR / f"grid_{args.phenomenon.lower()}_{args.date}_z500.png")
    plot_alpha_grid(base_ds, steered_ds_dict, 'msl', args.alphas, args.phenomenon,
                    output_path=AURORA_DIR / f"grid_{args.phenomenon.lower()}_{args.date}_msl.png")
