#!/bin/bash
#SBATCH --job-name=sp_ablation_hydro
#SBATCH --nodes=1
...
OUT_FILE="$HOME/aurora_thesis/thesis/benchmark/results_ifs_hydro/physics_evaluation_ifs_ablation_hydro_2020_ifs.csv"

# Ensure the output directory exists
mkdir -p "$(dirname "$OUT_FILE")"

echo "Running Ablation: dry_hydro..."
$PY $SCRIPT \
    --year 2020 \
    --model ifs_ablation_hydro \
    --prediction-zarr "$IFS_PRED_ZARR" \
    --reference ifs \
    --sp-ablation dry_hydro \
    --output "$OUT_FILE"
