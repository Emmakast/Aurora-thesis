...
MJO phases to identify where the model focuses differently during active events.
"""
...
OUTPUT_FIGURE = "mjo_contrastive_attention.png"
OUTPUT_FIGURE_MJO = "mjo_contrastive_attention_15N_15S.png"
...
def reduce_to_2d(tensor: torch.Tensor, mjo_only: bool = False) -> torch.Tensor:
    """
    ...
    Args:
        ...
        mjo_only: If True, apply latitude mask for MJO region (15N to 15S)
    ...
    # Check if we need to apply MJO latitudinal mask (15N to 15S)
    if mjo_only and ndim >= 4:
        ...
        if num_spatial == 1801 * 3600:  # 0.1 degree resolution
            lat_start, lat_end = 750, 1051  # (90 - 15)/0.1 to (90 + 15)/0.1
            lon_dim = 3600
        elif num_spatial == 721 * 1440:  # 0.25 degree resolution
            lat_start, lat_end = 300, 421   # (90 - 15)/0.25 to (90 + 15)/0.25
            lon_dim = 1440
        elif num_spatial == 180 * 360:  # 1 degree resolution
            lat_start, lat_end = 75, 106    # (90 - 15)/1 to (90 + 15)/1
            lon_dim = 360
        else:
            raise ValueError(f"Unknown spatial dimension size for MJO masking: {num_spatial}")
    ...
    if ndim == 2 or (mjo_only and ndim < 4):
        return tensor
    elif ndim == 3:
...
def aggregate_attention_for_dates(
    ...
    Args:
        ...
        mjo_only: Whether to mask for MJO area (15N - 15S)
    ...
    for i, (year, month, day) in enumerate(dates):
        try:
            ...
            tensor_2d = reduce_to_2d(tensor, mjo_only=mjo_only)
...
def plot_attention_line_plots(
    ...
    ax1.set_title("Baseline Attention Distribution (Neutral MJO Phase)", 
    ...
    ax2.set_title("Contrastive Attention: MJO Active vs Neutral", 
                  fontsize=14, fontweight='bold')
...
def main():
    ...
    mjo_df = df[df["Phenomenon"] == "MJO"].copy()
    print(f"  Found {len(mjo_df)} MJO dates out of {len(df)} total")
    
    # Step 2: Separate Active and Neutral dates
    print("\n[2/5] Separating Active and Neutral phases...")
    active_dates = mjo_df[mjo_df["Type"] == "Active"][["Year", "Month", "Day"]]
    neutral_dates = mjo_df[mjo_df["Type"] == "Neutral"][["Year", "Month", "Day"]]
    ...
    print(f"  Active MJO dates:  {len(active_list)}")
    print(f"  Neutral MJO dates: {len(neutral_list)}")
    ...
    for mode, mjo_mask, out_fig in [
        ("Global", False, OUTPUT_FIGURE),
        ("MJO-Region (15N-15S)", True, OUTPUT_FIGURE_MJO)
    ]:
        print(f"\n--- Processing {mode} Attention ---")
        
        # Step 3: Aggregate attention tensors
        print(f"\n[4/6] Aggregating Active phase attention ({mode})...")
        mean_active = aggregate_attention_for_dates(active_list, s3_client, mjo_only=mjo_mask)
        
        print(f"\n[5/6] Aggregating Neutral phase attention ({mode})...")
        mean_neutral = aggregate_attention_for_dates(neutral_list, s3_client, mjo_only=mjo_mask)
        ...
        suffix = "_mjo_region" if mjo_mask else ""
        results_path = f"mjo_contrastive_attention{suffix}.npy"
